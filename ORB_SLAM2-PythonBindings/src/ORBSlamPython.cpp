...
BOOST_PYTHON_MODULE(orbslam2)
{
    ...
    boost::python::class_<ORBSlamPython, boost::noncopyable>("System", boost::python::init<const char*, const char*, boost::python::optional<ORB_SLAM2::System::eSensor>>())
        ...
        .def("get_quaternion", &ORBSlamPython::getQuaternion)
        .def("get_euler", &ORBSlamPython::getEuler)
        .def("get_pose", &ORBSlamPython::getPose)
        ...
        .def("get_map_point", &ORBSlamPython::getMapPts)
        .def("get_reference_point", &ORBSlamPython::getRefPts)
        ...
        .def("SaveMap", &ORBSlamPython::SaveMap)
        .def("LoadMap", &ORBSlamPython::LoadMap)
        .def("get_frame", &ORBSlamPython::getFrame)
...
bool ORBSlamPython::SaveMap(std::string filename)
{
    system->SaveMap(filename);
    return true;
}
...
{
    system = std::make_shared<ORB_SLAM2::System>(vocabluaryFile, settingsFile, sensorMode, bUseViewer, map_file, load_map);
    return true;
}
...
cv::Mat ORBSlamPython::processStereo(cv::Mat leftImage, cv::Mat rightImage, double timestamp)
{
    if (!system)
    {
        cv::Mat Pose;
...
cv::Mat ORBSlamPython::processRGBD(cv::Mat image, cv::Mat depthImage, double timestamp)
{
    if (!system)
    {
        cv::Mat Pose;
...
boost::python::list ORBSlamPython::getPose()
{
    ...
    boost::python::list Pose;

    ORB_SLAM2::KeyFrame* pKF = vpKFs[vpKFs.size()-1];
    cv::Mat t = pKF->GetCameraCenter();
    
    Pose.append(t.at<float>(2));
    Pose.append(t.at<float>(0));
    Pose.append(t.at<float>(1));
    
    return Pose;
}

boost::python::list ORBSlamPython::getQuaternion()
{
    ...
    quat.append(q.coeffs().x());
    quat.append(q.coeffs().y());
    quat.append(q.coeffs().z());
    quat.append(q.coeffs().w());
                    
    return quat;
}

boost::python::list ORBSlamPython::getEuler()
{
    ...
    Euler.append(euler[0]); //roll
    Euler.append(euler[2]); //pitch
    Euler.append(euler[1]); //yaw
...
boost::python::list ORBSlamPython::getMapPts() const
{
    if (!system)
    {
        return boost::python::list();
    ...
    // This is copied from the ORB_SLAM2 System.SaveTrajectoryKITTI function, with some changes to output a python tuple.
    vector<ORB_SLAM2::MapPoint*> Mps = system->GetAllMapPts();
...
boost::python::list ORBSlamPython::getRefPts() const
{
    if (!system)
    {
        return boost::python::list();
...
cv::Mat ORBSlamPython::getFrame()
{
    cv::Mat OrbFrame = system->GetOrbFrame();
    return OrbFrame;
}
